...
for node in $NODES; do
    ...
    if [ "${PORT}" == "22" ]; then
        ...
        ssh -p $PORT johren@$HOST "sudo wget https://github.com/downloads/GENI-GEMINI/GEMINI/gemini-gn-active-ubuntu10-20120531.tar.gz"
        ssh -p $PORT johren@$HOST "sudo tar -zxvf gemini-gn-active-ubuntu10-20120531.tar.gz"
        echo "   Installing Shared-Ubuntu.sh" 
        ssh -p $PORT johren@$HOST "sudo ./Shared-Ubuntu.sh"
        ...
        ssh -p $PORT johren@$HOST "sudo /usr/local/etc/lamp/bootstrap.sh ${SLICEURN} ${USERURN}"
        echo "   Installing apache2-Ubuntu.sh" 
        ssh -p $PORT johren@$HOST "sudo ./apache2-Ubuntu.sh"
        echo "   Installing perfSONAR_PS-ServiceWatcher-Ubuntu.sh" 
        ssh -p $PORT johren@$HOST "sudo ./perfSONAR_PS-ServiceWatcher-Ubuntu.sh"
        echo "   Installing perfSONAR_PS-Toolkit-Ubuntu.sh" 
        ssh -p $PORT johren@$HOST "sudo ./perfSONAR_PS-Toolkit-Ubuntu.sh"
      
    else
        echo "Installing MP software on node $HOST port $PORT" 
        ssh -p $PORT johren@$HOST "sudo wget https://github.com/downloads/GENI-GEMINI/GEMINI/gemini-mp-active-fedora15-20120531.tar.gz"
        ssh -p $PORT johren@$HOST "sudo tar -zxvf gemini-mp-active-fedora15-20120531.tar.gz"
        echo "   Installing Shared-Fedora.sh" 
        ssh -p $PORT johren@$HOST "sudo ./Shared-Fedora.sh"
        echo "   Installing LAMP certificate" 
        ssh -p $PORT johren@$HOST "sudo install -o root -g perfsonar -m 440 lampcert.pem /usr/local/etc/protogeni/ssl/"
        echo "   Running bootstrap" 
        ssh -p $PORT johren@$HOST "sudo /usr/local/etc/lamp/bootstrap.sh ${SLICEURN} ${USERURN}"
        echo "   Installing mysql-Fedora.sh" 
        ssh -p $PORT johren@$HOST "sudo ./mysql-Fedora.sh"
        echo "   Installing perfSONAR_PS-ServiceWatcher-Fedora.sh" 
        ssh -p $PORT johren@$HOST "sudo ./perfSONAR_PS-ServiceWatcher-Fedora.sh"
        echo "   Installing perfSONAR_PS-psConfig-Fedora.sh" 
        ssh -p $PORT johren@$HOST "sudo ./perfSONAR_PS-pSConfig-Fedora.sh"
        echo "   Installing perfSONAR_PS-LSRegistrationDaemon-Fedora.sh" 
        ssh -p $PORT johren@$HOST "sudo ./perfSONAR_PS-LSRegistrationDaemon-Fedora.sh"
        echo "   Installing perfSONAR_PS-perfSONARBUOY-Fedora.sh" 
        ssh -p $PORT johren@$HOST "sudo ./perfSONAR_PS-perfSONARBUOY-Fedora.sh"
        echo "   Installing perfSONAR_PS-PingER-Fedora.sh" 
        ssh -p $PORT johren@$HOST "sudo ./perfSONAR_PS-PingER-Fedora.sh"
    fi
done
